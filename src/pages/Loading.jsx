...
export default function Loading() {
  ...
  const [progress, setProgress] = useState('사진 분석 준비중…');
const hasRunRef = useRef(false);               // 🔑 StrictMode 가드

  /* Mixpanel */
  useEffect(() => {
    const id = localStorage.getItem('mixpanel_distinct_id') ?? crypto.randomUUID();
 ...
 hasRunRef.current = true;
    ...
    const runAnalysis = async () => {
      try {
        ...
        const fJson = await (await fetch(`${API_BASE}/analyze/features/`, { method:'POST', body:form })).json();
        if (fJson.error) throw new Error(fJson.error);
        const features = fJson.features.trim();
        if (features === 'again') throw new Error('얼굴이 인식되지 않았습니다.');

        /* 2) mini(detail1~3) */
        setProgress('재물운 보고서 작성 중…');
        const miniJson = await (await fetch(`${API_BASE}/analyze/wealth/mini`, {
          method:'POST',
        ...
        /* 3) summary + score */
        setProgress('점수·요약 계산 중…');
        const scoreJson = await (await fetch(`${API_BASE}/analyze/wealth/score`, {
          method:'POST',
          headers:{ 'Content-Type':'application/json' },
          body:JSON.stringify({
        ...
        /* 5) 세션 백업 & 이동 */
        sessionStorage.setItem('wealth_result',
        ...
        <div className={styles.border_wrap}>
          <div className={styles.border}>
            ...
            <div className={styles.image_title_wrap}>
              <div className={styles.ai}>{progress}</div>
            </div>
          </div>
        </div>
      </div>
